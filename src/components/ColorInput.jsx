...
const ColorInput = ({
  ...
  if (format === 'plain') {
    ...
    field = (
      <Box ref={forwardRef} sx={{ display: 'flex', flexDirection: 'row' }} component="div">
        {names.map(cn => (
          <FormControl key={cn} className="muicc-colorinput-raw" error={!!color.error} sx={{ pr: 4 }}>
  ...
  return (
    <FormControl sx={{ width: 100 }} error={!!color.error} data-testid="colorinput">
      {field}
